...
export default {
    data() {
        return  {
            ...
            entry: '', // 登录权限
            ids: '', // 角色id字符串
            projectOrCompanyOption: [], // 所属项目选项
            projectOrCompanyId: '', // 所属项目
            userStateOptions: [
                {
                    value: '1',
                    label: '启用'
    ...
    methods: {
        ...
        // 获取账号列表
        querySystemUserList () {
            this.$axios.post(`/api/system/computer/querySystemUser?userType=${this.userType}&parameterId=${this.cid}&page=${this.page}&pageSize=${this.pageSize}`).then(
                res => {
        ...
        // 获取项目列表
        getProjectList() {
            this.$axios.post(`/api/project/selectProjectList?cid=${this.cid}&pageNum=1&pageSize=9999`).then(
                res => {
                    // console.log(res.data)
