<template>
    <div id="systemLiangZhi_permission_user">
...
<style lang="less">
    #systemLiangZhi_permission_user {
        width: 100%;
        .centent-box {
            border-radius: .04rem;
...
export default {
    ...
    methods: {
        // 获取项目id
        getProjectId() {
            this.parameterId = sessionStorage.getItem('pid')
            this.belong = sessionStorage.getItem('pid')
        ...
        // 获取账号列表
        querySystemUserList() {
            this.$axios.post(`/api/system/computer/querySystemUser?userType=${this.userType}&parameterId=${this.parameterId}&page=${this.page}&pageSize=${this.pageSize}`).then(
                res => {
                    // console.log(res.data)
                    let temp = []
        ...
        querySystemUser() {
            if (this.userName && this.userPhone && this.userAccount && this.userPassword && this.userState && this.entry) {
                this.$axios.post(`/api/system/computer/insertSystemUser?projectOrCompanyId=${this.parameterId}&publicId=${this.parameterId}&userName=${this.userName}&userPhone=${this.userPhone}&userAccount=${this.userAccount}&userPassword=${this.$md5(this.userPassword)}&userState=${this.userState}&userType=${this.userType}&entry=${this.entry}&ids=${this.ids}`).then(
                    res => {
                        if (res.data.code == 0) {
                            this.$message({
                                message: '添加成功',
        ...
        updateSystemUser() {
            if (this.userName && this.userPhone && this.userAccount && this.userState && this.entry) {
                ...
                this.$axios.post(`/api/system/computer/updateSystemUser?publicId=${this.parameterId}&userName=${this.userName}&userPhone=${this.userPhone}&userAccount=${this.userAccount}&userPassword=${this.userPassword?this.$md5(this.userPassword):''}&userState=${this.userState}&userType=${this.userType}&entry=${this.entry}&ids=${this.ids}&id=${this.id}`).then(
                    res => {
                        if (res.data.code == 0) {
                            this.$message({
                                message: '修改成功',
        ...
        // 搜索
        searchClick() {
            this.$axios.post(`/api/system/computer/querySystemUser?id=${this.userId}&parameterId=${this.parameterId}&page=1&pageSize=${this.pageSize}&userName=${this.searchUserName}`).then(
                res => {
                    let temp = []
                    for (let i = 0; i < res.data.data.rows.length; i++) {
                        temp.push({
        ...
        // 翻页
        pageClick() {
            this.$axios.post(`/api/system/computer/querySystemUser?id=${this.userId}&parameterId=${this.parameterId}&page=${this.page}&pageSize=${this.pageSize}&userName=${this.searchUserName}`).then(
                res => {
                    let temp = []
                    for (let i = 0; i < res.data.data.rows.length; i++) {
                        temp.push({
