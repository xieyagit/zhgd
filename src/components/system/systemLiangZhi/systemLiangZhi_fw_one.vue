    ...
    <div class="content">
      ...
      <div class="table-box">
        ...
        <el-table :data="tableData2" stripe border>
          ...
          <el-table-column prop="materialName" label="资料名称" width="400"></el-table-column>
          <el-table-column prop="uploadingname" label="上传人" width="130"></el-table-column>
          <el-table-column prop="uploadingDate" label="上传时间" width="200"></el-table-column>
          <el-table-column prop="status" label="状态" width="80" sortable="custom">
            <template slot-scope="scope">
              <div
                :class="scope.row.uploadingType=='已上传'?'green-color':scope.row.uploadingType=='未上传'?'red-color':''"
              >{{ scope.row.uploadingType }}</div>
            </template>
          </el-table-column>
          <el-table-column label="操作" align="right">
            <template slot-scope="scope">
              ...
              <el-button
                type="text"
                size="small"
                v-if="scope.row.uploadingType=='已上传'"
                @click="getParticulars(scope.row.id);lookInfo=true"
              >点击查看</el-button>
            </template>
          </el-table-column>
      ...
      <!-- 上传弹框 -->
      <div id="uploading">
        <el-dialog :title="uploadingTitle" :visible.sync="uploading" @close="fileList = [],remark='',file=''" width="34.5%">
            ...
            <div class="checkImg">
              ...
              <div class="lxsupserSb" v-if="file !=''">
                <div class="info">
                  ...
                  <el-tooltip content="最多一次性添加30M内容！" placement="top">
                    <span class="addBtn">
                      重新添加
                      <input type="file" multiple class="lxSbfile" @change="fileUp" />
                    </span>
                  </el-tooltip>
                </div>
                <div class="imgLook">
          ...
          <span slot="footer" class="dialog-footer">
            <button class="uploadingBtn" style="backgroundColor:gray;border:1px solid gray" v-if="loading">正在上传...</button>
            <button @click="uploadingClick" class="uploadingBtn" v-else>确认上传</button>
            <button @click="uploading = false,fileList = [],remark='',file=''" class="closeBtn">取 消</button>
          </span>
        </el-dialog>
      </div>
      <!-- 查看弹框 -->
  ...
  methods: {
    ...
    fileUp(e) {
      ...
      setTimeout(() => {
        for (let i = 0; i < readList.length; i++) {
          var filesType = e.target.files[i].name.slice(e.target.files[i].name.lastIndexOf('.') + 1)
          this.fileList.push({
            src: filesType == 'xls'? require('../../../../static/images/xls.png') : filesType == 'xlsx'? require('../../../../static/images/xls.png') : filesType  == 'png' ? readList[i].result : filesType  == 'jpg' ? readList[i].result : filesType  == 'gif' ? readList[i].result : filesType  == 'jpeg' ? readList[i].result : filesType == 'docx'? require('../../../../static/images/word.png') : filesType == 'doc'? require('../../../../static/images/word.png') : filesType == 'pdf'? require('../../../../static/images/pdf.png') : readList[i].result,
    ...
    uploadingClick() {
      ...
      if (this.file) {
        ...
        this.$axios.post(`/api/lzfw/instadd?menuId=${this.uploadingId}&uploadingName=${this.userName}&projectId=${this.projectId}&remark=${this.remark}`,this.file,headers)
        .then(
          res => {
            // console.log(res.data)
