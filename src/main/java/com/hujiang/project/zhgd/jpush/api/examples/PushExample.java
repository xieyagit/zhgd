...
import com.hujiang.project.zhgd.jpush.api.JPushClient;
import com.hujiang.project.zhgd.jpush.api.push.CIDResult;
import com.hujiang.project.zhgd.jpush.api.push.GroupPushClient;
import com.hujiang.project.zhgd.jpush.api.push.PushResult;
import com.hujiang.project.zhgd.jpush.api.push.model.*;
import com.hujiang.project.zhgd.jpush.api.push.model.audience.Audience;
import com.hujiang.project.zhgd.jpush.api.push.model.audience.AudienceTarget;
import com.hujiang.project.zhgd.jpush.api.push.model.notification.*;
...
//    protected static final String MASTER_SECRET = "860803cf613ed54aa3b941a8";
    protected static final String APP_KEY ="0c578ef6b15297568e54bb0b";
    protected static final String MASTER_SECRET = "e09d65cb1daf863ba1f585ab";
...
//	}
	...
	public void testSendPush(PushPayload payload) {
		ClientConfig clientConfig = ClientConfig.getInstance();
...
//        ApacheHttpClient httpClient = new ApacheHttpClient(authCode, null, clientConfig);
    ...
    public void testSendGroupPush() {
      ...
      //根据别名推送至指定用户
    public static PushPayload buildPushObjectAllRegistrationIdAlertWithTitle(String alert, String alias, String title,
                                                                             boolean isApnsProduction,Map<String,String> extras
    ) {
        ...
        return PushPayload.newBuilder()
                ...
                .setNotification(Notification.newBuilder()
                        ...
                        .addPlatformNotification(AndroidNotification.newBuilder()
                                .setAlert(alert)         //通知内容
                                .setTitle(title)        //通知标题
                                .setBuilderId(0)    //通知栏样式 ID
                                .setPriority(0)         //通知栏展示优先级
                                .setCategory(null)          //通知栏条目过滤或排序
                                .setStyle(0)                //通知栏样式类型
                                //.setAlertType()        //通知提醒方式
                                //.setBigText(bigText)            //大文本通知栏样式:当 style = 1 时可用，内容会被通知栏以大文本的形式展示出来。
//                                .setInbox(inbox)                //文本条目通知栏样式
//                                .setBigPicPath(bigPicPath)      //大图片通知栏样式
                                .addExtras(extras)         //扩展字段
//                                .setLargeIcon(largeIcon)        //通知栏大图标
                             //   .setIntent(intent)              //指定跳转页面
                                .build())
                        //指定当前推送的iOS通知
...
//                                .setThreadId(threadId)
	...
	/** 所有平台，推送目标是别名为“alias1”，通知内容为ALERT */
    public static PushPayload buildPushObject_all_alias_alert() {

        return PushPayload.newBuilder()
                .setPlatform(Platform.all())
                .setAudience(Audience.alias("legendary"))
    ...
    /** 平台是Android，目标是标签为“tag1”的设备，内容是Android通知ALERT，并且标题为TITLE */
    ...
    /** 平台是iOS，推送目标是“tag1”，“tag_all”的交集，
     * 推送内容同时包括通知与消息 - 通知信息是ALERT，
     * 角标数字为5，通知声音为“happy”，并且附加字段from =“JPush”;
     * 消息内容是MSG_CONTENT。 */
    ...
    /** 平台是Andorid与iOS，推送目标是（“tag1”与“tag2”的并集）交（“alias1”与“alias2”的并集），
     * 推送内容是 - 内容为MSG_CONTENT的消息，并且附加字段from = JPush */
